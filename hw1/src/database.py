...
"""CATEGORY: name, id, listing_count"""
CATEGORY_CACHE = tuple[int, List[str]]
"""CATEGORY_CACHE: max_count, top_category_names"""
LISTING = tuple[int, str, str, float, str, str, str]
"""LISTING: id, title, description, price, created_at, username, category"""
...
class Database:
    ...
    def initialize(self):
        try:
            ...
            print("Error - database already initialized")
    ...
    def is_user_exist(self, username):
        return (
            self.get_user_by_name(username) is not DatabaseResponse.GET_OBJECT_NOT_FOUND
        )
    ...
    def get_category_by_name(self, category_name) -> CATEGORY:
        category = self.__CURSOR.execute(
    ...
    def is_category_exist(self, category_name):
        return (
            self.get_category_by_name(category_name)
            is not DatabaseResponse.GET_OBJECT_NOT_FOUND
        )

    def get_top_categories(self) -> CATEGORY_CACHE:
        max_count = self.__CURSOR.execute(
            "SELECT MAX(listing_count) FROM categories"
        ).fetchone()[0]
        if max_count is None or max_count == 0:
            return None

        top_categories = self.__CURSOR.execute(
            "SELECT * FROM categories WHERE listing_count = ? ORDER BY name ASC",
            (max_count,),
        ).fetchall()
    ...
    def get_listings_by_category(self, category_name) -> List[LISTING]:
        listings = self.__CURSOR.execute(
    ...
    def update_top_categories_cache(self, category_name, new_listing_count):
        ...
        if new_listing_count > self.__TOP_CATEGORIES_CACHE[0]:
            self.__TOP_CATEGORIES_CACHE = (new_listing_count, [category_name])

        elif new_listing_count == self.__TOP_CATEGORIES_CACHE[0]:
            self.__TOP_CATEGORIES_CACHE[1].append(category_name)

        elif (
            category_name in self.__TOP_CATEGORIES_CACHE[1]
            and new_listing_count < self.__TOP_CATEGORIES_CACHE[0]
        ):
            self.__TOP_CATEGORIES_CACHE = self.get_top_categories()
